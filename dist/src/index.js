...
import { getApplicationName, getBundleId, getReadableVersion } from 'react-native-device-info';
...
var ExpoMixpanelAnalytics = (function () {
    function ExpoMixpanelAnalytics(token) {
        ...
        Constants.getWebViewUserAgentAsync().then(function (userAgent) {
            ...
            _this.appName = getApplicationName();
            _this.appId = getBundleId();
            _this.appVersion = getReadableVersion();
    ...
    ExpoMixpanelAnalytics.prototype._pushEvent = function (event) {
        var data = {
            event: event.name,
            properties: tslib_1.__assign({}, (event.props || {}), this.superProps)
        };
        if (this.userId) {
